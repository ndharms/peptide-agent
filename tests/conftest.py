...
def tmp_repo(tmp_path: Path):
    ...
    (papers / "metadata.json").write_text(
        json.dumps({"DOI": "10.1234/example", "PAPER_TYPE": "RESEARCH"}),
        encoding="utf-8",
    )
...
def stub_heavy_deps(monkeypatch):
    ...
    class StubLoader:
        ...
        def load(self):
            ...
            return [
                FakeDoc("Methods: peptide FF assembled into fibers at pH 7.0 in water.")
            ]

    monkeypatch.setattr("src.pipeline.agent.PyPDFLoader", StubLoader, raising=True)
    ...
    def fake_from_documents(docs, _emb):
        ...
        return FakeVectorStore(
            holder.retrieval_docs if hasattr(holder, "retrieval_docs") else docs
        )

    monkeypatch.setattr(
        "src.pipeline.agent.FAISS.from_documents",
        staticmethod(fake_from_documents),
        raising=True,
    )
